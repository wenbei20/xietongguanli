<template>
  <div class="xuqiu_container XQ_and_QX">
    <div class="list-head">
      <div class="btns">
        <el-button type="primary" size="medium" @click="showAddDialog = true"> <i class="el-icon-plus" /> 创建批次</el-button>
        <el-select v-model="caozuo" placeholder="更多操作">
          <el-option label="批次统计" value="1" />
          <el-option label="导入批次" value="2" />
          <el-option label="导出批次" value="3" />
        </el-select>
    ...
    <div class="table">
      ...
      >
        ...
        <vxe-table-column field="title" title="批次名称" tree-node width="400" :edit-render="{}">
        ...
        <vxe-table-column field="version" title="批次年度" tree-node width="100">
        ...
        <vxe-table-column field="miaoshu"  title="批次描述"  width="200" :edit-render="{}">
          

          <template #default="{ row }">
            <span class="titleText">{{row.miaoshu}}</span>
          </template>
           <template slot="edit" slot-scope="scope">

            <el-input v-model="scope.row.miaoshu" size="mini" />
          </template>
        ...
        <create :dialogVisible.sync="showAddDialog"  />
  </div>
</template>
